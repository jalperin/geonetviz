		...
		// I understand JS' "this" keyword. Is it "right" to use "map" here instead?
			...
			arcs
              .enter().append("path")
 			    .attr("class", "arc")
            	.attr("d", function(d) {
					var tmp_src = map.country_group.selectAll('[country_code=' + network.nodes[d.source].id + ']');
					var tmp_tgt = map.country_group.selectAll('[country_code=' + network.nodes[d.target].id + ']');
